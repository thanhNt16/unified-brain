...
from .projection import index_all
from .projection import rebuild as rebuild_projection
...
def run(vault: Vault, rebuild: bool = False) -> tuple[dict[str, object], int]:
    ...
    if rebuild:
        projected = rebuild_projection(vault)
        errors = int(projected["errors"])
        data: dict[str, object] = dict(projected)
    else:
